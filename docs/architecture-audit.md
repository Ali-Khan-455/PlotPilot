...
1. **Chapter detection on messy `.txt`.** A heading is a line that is either the first line or follows a blank line, **and** is one of:
   ...
   - A heading whose number is followed by a lowercase word (`Chapter 12 the fall`) is not detected, for all three number forms. That chapter merges into the one before it.
