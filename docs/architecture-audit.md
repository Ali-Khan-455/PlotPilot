...
| R21 | Phase 4 rulings | **Chunk loop:** the current chunk is the first one not `done`. A run advances to the next chunk after an accept. **Operator flags bind only to the run-start chunk**; the next chunk runs with none, so it always stops at its own module gate (R7, D2). `--accept-tracker` is decided at run start: it applies only if that chunk was `tracker_pending` and no edit was recorded, so a delta produced in the same run is never merged (D4). `--redraft` or `--repair-margin` together with `--accept-tracker` is refused, so the operator always reviews a changed chunk before its tracker is merged. `--redraft` on a `planned` chunk is a no-op with a note. `--module` on a drafted chunk is ignored unless `--redraft` is also given. `--repair-margin` is ignored once chunk 1 is `done`. **Tracker:** stored as JSON in `tracker_versions`: `UNIQUE(chunk_id)`, the merged `json`, the exact accepted `delta`, and `add_tracker_version` inserts the row and sets `done` atomically. Title, chunk boundaries, progress and overrides are derived from other tables, not stored. **Merge:** characters are deduped by name, terms by term (their `chunk` is forced to the actual chunk), comparisons and motifs by normalized text; `last_state` = `chunk_end_state`. Collisions (the model's list plus a deterministic stand-in check) are shown at review only and never stored. **Render:** one function whose layout mirrors the spec template's headings; it is data formatting, not prompt text. The prompt form never includes fact-check overrides (R15). The mirror `trackers/<slug>.md` includes them. It is derived, read-only and regenerated after every accept, so operator edits to it are lost; the review surface is the pending JSON. MC voice notes render only "Asides used so far" (the motifs). Chunk 1's prompts get `EMPTY_TRACKER`; later chunks get the rendered tracker. **Pending file:** `trackers/<slug>.chunk-NN.delta-<pass_id>.pending.json`, bound to the latest ok delta pass. Only that file is shown or merged. Other `<slug>.chunk-*.delta-*.pending.json` files are deleted at the gate, with a "superseded" note if a same-chunk file's parsed JSON differs from its pass (an operator edit). **Prompts 10/11** rerun only after a body-changing pass (`draft`, `operator_edit`, `texture`, `tts`; not `margin_repair`). **Done chunks are frozen:** this overturns §3.4's "an edit at any later status" for `done`. An edited file is warned about and ignored (the stored text is used; delete the file to restore it), and a missing file is restored. **Chunk 1 tracker fields:** at accept, `margin`, `margin_sentences = count_sentences(margin)` and `target = first_sentence(final body)` (R20). `first_sentence` ends a sentence at `.`, `!` or `?` (plus an optional closing quote) only before an uppercase letter, an opening quote, or the end. So `He said "Run." and left.` is one sentence; common abbreviations (`Mr. Mrs. Ms. Dr. St. Jr. Sr. vs. etc.`, case-sensitive) never end a sentence. Known limits: `etc.` at a real sentence end doesn't end it (the target just gets longer and is still a prefix of the body), and uppercase `MR.` still splits. **Labels and progress** use the book's own numbering (`Ch 1–5`, `Prologue–Ch 4`, `Ch 5–Epilogue`); the tracker's progress line is cumulative (`Chapters 1–6 processed so far`, `… and part 1/2 of Chapter 6` after a split chapter's non-final part). **Output streams:** errors go to stderr; gates, notes, warnings and progress go to stdout. **Files:** every chunk file ends with exactly one newline. Chunks 2+ contain the body only. | Pipeline only. |
| R22 | Phase 5 rulings | **Hook passes:** `hook` (Prompt 5) and `hook_tts` (Prompt 9 on the hook, R9) are passes on chunk 1; chunk 1 stays `done`. `hook_tts` is its own kind because `tts` would enter chunk 1's narration fold. Prompt 5 runs once: it is skipped whenever an ok `hook` pass exists, and there is no redo flag (`--redraft`, `--repair-margin`, `--module` are already ignored once every chunk is done). A failed Prompt 9 on the hook reruns only that pass. **Prompt 5 input:** system = Prompt 1 + chunk 1's module (stored on the hook pass); user = Prompt 5 with the tracker's `chunk1.target` and the pre-hook narration (chunk 1 with its placeholder margin, then every body). **D17:** `messages.count_tokens` on the full request; stop when tokens + `HOOK_MAX_TOKENS` exceed the model's `max_input_tokens` from `models.retrieve` (anthropic 1.8's `ModelInfo.max_input_tokens`, `Optional[int]`; `GEN_CONTEXT_TOKENS` only when it is `None`). N in the message is the pre-hook word count; M counts the whole request. **D20:** the target check (chunk 1's body starts with the target) runs before Prompt 5, so a hook is never generated against a bad target; the splice check then runs on the assembled script (paragraph 2 starts with the target, and the hook doesn't end with it). Hook-vs-target duplication means the hook's last N normalized words equal the target's; sharing a phrase is fine. `hook length` text inside the hook is rejected. **Script:** `<out>/<slug>/script.txt` = hook as its own first paragraph, then each chunk body. `chunk-01.txt` keeps its placeholder margin (done chunks are frozen). Script and metadata are derived artifacts, rebuilt from SQLite on every run; a file that differed (a hand edit) gets a warning before it is regenerated. Prompt 9 on the hook uses Prompt 9's ratio but may always drop one word (a homograph rewrite). If `count_tokens` itself refuses the request (400), that is reported as D17 with a word-based token estimate. **Metadata (R5):** `metadata/<slug>.txt`. Words are compared after lowercasing and dropping every non-alphanumeric character. Each chunk's latest ok scenes are searched forward from a cursor (`hit + 1`) and bounded to that chunk's word range. Offset × 60 // 150 wpm → `[mm:ss]`, minutes uncapped. The first scene is always `[00:00]`. An unfound scene takes the previous timestamp and a printed warning (not written to `logs/errors.log`, since the file is rebuilt every run). | Pipeline only. |
...
plotpilot/
  ...
  final.py              stages 6–8: D17 check, hook (P5 → P9), splice check, script + metadata files
...
- `passes(id, novel_id, chunk_id NULL, kind, model, module NULL, input_text, output_text, verdict NULL, note NULL, created_at)`
  `kind` ∈ `classify, draft, margin_repair, audit, factcheck, factcheck_override, operator_edit, texture, tts, tracker_delta, scenes, hook, hook_tts`. `db.add_pass(..., new_status=)` inserts a pass and sets `chunks.status` in one transaction.
...
| hook_tts | P9 | Haiku | hook | plain text |
...
1. **Chapter detection on messy `.txt`.** A heading is a line that is either the first line or follows a blank line, **and** is one of:
   - `Chapter`/`Ch.` followed by a numeral, a valid roman numeral, or a number word such as `Twenty-One` or `One Hundred and One`. After the number (any of the three forms) the line must end, or continue with `: . - – — ,`, `(`, `[`, a quote, or a word starting with an uppercase letter (`Chapter 1 The Beginning`, `CHAPTER 12 THE FALL`), with the rest of the line ≤80 chars. So `Chapter one of my life was over.` and `Chapter 12 of the regulations forbade it.` are prose, not headings;
   ...
   - A heading whose number is followed by a lowercase word (`Chapter 12 the fall`) or a non-ASCII capital (`Chapter 3 Élan`) is not detected, for all three number forms. That chapter merges into the one before it.
   - Prose after a blank line whose number is followed by an uppercase word or an opening bracket is taken as a heading: `Chapter 3 I think was best.`, `Chapter 12 The regulations forbade it.`, `CHAPTER 12 WAS THE BEST ONE.`, `Chapter 12 (of the rules) forbade it.`. The sequence warning catches it only when the number goes backwards or repeats.
